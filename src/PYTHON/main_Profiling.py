...
pr = profile.Profile()
for i in range(5):
    calibrate_profile.append(pr.calibrate(10000))
print sum(calibrate_profile) / float(len(calibrate_profile))

# Assummes at list 0.1seconds to run
import time
...
#    InputOutput = io_ljmd.Io_ljmd('..')
    ...
    parameters = read_inputs.read()#AGREGAN)
