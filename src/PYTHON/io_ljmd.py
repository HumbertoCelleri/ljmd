...
class Read_inputs(Io_ljmd):
        ...
        def read(self): # AGREGAN POR DEFAULT y sacamos readline-> ASIGNAMOS
            ...
            from inputs_md import *
            input_dict={}
            input_dict['natoms'] = natoms  # Number of atoms
            input_dict['mass'] = mass      # Mass in AMU
            input_dict['rcut'] = rcut      # Cut-off radius in angstrom
            input_dict['box'] = box        # Box length in angstrom
            if potential == 'lj':
                input_dict['epsilon'] = epsilon  # Epsilon in kcal/mol
                input_dict['sigma'] = sigma      # Sigma in angstrom
                input_dict['method'] = 0
            elif potential == 'morse':
                input_dict['D_e'] = D_e  # D_e in kcal/mol
                input_dict['r_e'] = r_e  # r_e in angstrom
                input_dict['a'] = a      # a in 1/angstrom
                input_dict['method'] = 1
            input_dict['restfile'] = restfile  # Filename for restart
            input_dict['trajfile'] = trajfile  # Filename for trajectories
            input_dict['ergfile'] = ergfile    # Filename for energies
            input_dict['nsteps'] = nsteps      # Number of MD steps
            input_dict['dt'] = dt              # MD time step (in fs)
            input_dict['nprint'] = nprint      # Output print frequency
            #ifile.close()
